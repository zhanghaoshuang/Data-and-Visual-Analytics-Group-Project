SELECT PHI_B_DEM.POSTAL_CODE ZIP_CODE, PHI_CAT.NAME CATEGORY,
	COUNT(PHI_B_DEM) BI_NUM, 
	AVG(PHI_B_DEM.STARS) STARS, AVG(PHI_B_DEM.COMP_STARS) COMP_STARS, 
	SUM(PHI_B_DEM.REVIEW_COUNT) REVIEW_COUNT, SUM(PHI_B_DEM.COMP_REVIEW_COUNT) COMP_REVIEW_COUNT, 
	SUM(PHI_B_DEM.TOTAL_DEMAND_SCORE) TOTAL_DEMAND_SCORE, SUM(PHI_B_DEM.MISSING_DEMAND_SCORE) MISSING_DEMAND_SCORE
FROM PHI_CATEGORY_V1 PHI_CAT, PHI_BI2CAT PHI_B2C, (
...
WHERE (PHI_B_DEM.ID = PHI_B2C.ID) AND (PHI_B2C.CATV1_ID = PHI_CAT.ID)
GROUP BY PHI_B_DEM.POSTAL_CODE, PHI_CAT.NAME
ORDER BY ZIP_CODE, MISSING_DEMAND_SCORE DESC